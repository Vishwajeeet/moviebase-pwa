...
state.playlistId = new URLSearchParams(
  window.location.search).get('playlistId');
...
function showStep(n) {
  document.querySelectorAll('.step').forEach(
    function(s) { s.classList.remove('active'); }
  );
...
function doSearch(query) {
  ...
  fn(query).then(function(results) {
    ...
    console.error(err);
    document.getElementById('search-status')
      .textContent = 'Search failed. Check connection.';
  });
}
...
function renderResults(results) {
  ...
  if (results.length === 0) {
    container.innerHTML =
      ...
      + ' style="padding:12px 0">No results found.</p>';
    return;
  }
...
function loadSeasons(tmdbId) {
  ...
  list.innerHTML = '<p class="text-muted text-sm"'
    + ' style="padding:16px 0">Loading seasons...</p>';
...
function showStep3() {
  ...
  var posterUrl = AppUtils.getPosterUrl(
    s ? s.poster : r.poster
  );
  var previewPoster = document.getElementById(
    'preview-poster');
  if (posterUrl) {
  ...
  document.getElementById('preview-sub')
    .textContent = s ? s.seasonName : 'Movie';
  ...
  .addEventListener('click', function() {
    ...
    function saveEntry(runtime, genres) {
      var entry = {
        ...
        title: r.title,
        type: r.type,
        season: s ? s.seasonNumber : null,
        seasonName: s ? s.seasonName : null,
        poster: s
          ? (s.poster || r.poster)
          : r.poster,
        releaseYear: s
        ...
        yearWatched: AppUtils.getCurrentYear(),
    ...
    if (r.type === 'movie') {
      AppTMDB.getMovieDetails(r.tmdbId)
        .then(function(details) {
          saveEntry(details.runtime, details.genres);
        }).catch(function() {
          saveEntry(90, []);
        });
    } else {
      ...
      Promise.all([
        ...
        saveEntry(avgRuntime * epCount, genres);
      }).catch(function() {
